...
7. **Decide builder questions yourself**, unless the answer faces outward. Run each belt as
   ...
   issues.
8. **Review each lane as it merges, effort `high`.** Dispatch `reviewer` over that lane's
   range at once. It writes that feature's review record. Land its MAJOR-and-worse findings
   while the other lanes build, then run `make sdlc ARGS='close feature <id>'`.
9. **A milestone checkup, at lower effort, after the last lane closes.** One `reviewer`
   checks the ship criterion and the cross-lane seams: duplication, drift between lanes, a
   config or doc census that one lane's key breaks. It does not repeat the lane reviews. It
   writes only the milestone record.
10. **Review the fix commits before release.** One `reviewer` over the commits that landed
   findings asks one question: does each fix close its finding without a new defect? It
   appends its block to the milestone record. Then run `make sdlc ARGS='release <version>'`.
   Nothing runs above `high`.
11. **Measure every dispatch** when it returns, against the previous milestone. The
