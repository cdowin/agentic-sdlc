...
- **nullbound** — pinned `DEVKIT_VERSION := v0.23.0`. Must not bump until the tag exists.
- **trail** — pinned v0.23.0. **No longer blocks anything.** Its one D5 drift only ever reddened
...
12. **Decide the history.** Chris on the plain copy: *"not having history is a bit of a boon because we
    have so much pollution from the trail and nullbound consumer smoke checks."* `pm/` and `CHANGELOG.md`
...
19. **nullbound** — bump `DEVKIT_VERSION` to `v0.24.0`; `install-ci --diff` then decide **PER FILE**
...
20. **trail** — the same. Its deliberate two-job sharded `verify.yml` is 177 lines from the installable;
...
| 4 | trail's D5 drift (`dossier-script-split` story at `review` under a `planning` feature) | nothing. Fix at leisure. |
...
- **A gate reading a half-deleted tree can produce a confident WRONG verdict**, not just a crash. Measured:
  `behavior-fan-scan` in nullbound called three live allowlist entries stale while `awk` could not open a
