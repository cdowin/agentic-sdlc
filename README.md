...
| `install-sdlc` | `docs/sdlc-protocol.md` — the one plan entry that is **RENDERED rather than copied**, from your own `[story]` / `[feature]` / `[release]` / `[adopt]` check lists and the `[pm.states.<kind>] done` state each belt writes, so the document a human reads cannot drift from the checks the belts run. Re-run it after changing a check list |
