...
| `install-agents` | `.claude/agents/`: the four the loop dispatches — architect, developer, reviewer, tech-writer — each pointing at `make sdlc ARGS='dispatch …'` for the ladder, gate roster and vocabulary rather than carrying a hand-edited copy, with the judgement calls the tool cannot derive left yours after install |
