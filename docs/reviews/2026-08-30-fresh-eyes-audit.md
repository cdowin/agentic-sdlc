...
- Config-over-forks never landed for the read verbs: `refs.py:29` hardcodes `ALWAYS_EXCLUDED`
  ...
  consumer_a's exact directory roster. Six files import `load_config` and never call it (dead
...
- `tests/test_tscn_roundtrip.py` consumer sweep is `@skipUnless(available_consumers())` —
  `~/workspace/{consumer_a,consumer_b}` absent on ubuntu-latest, so CI proves the hermetic fixture only;
...
- `repo/pm/templates/__init__.py:27-31, 98-157, 254-312, 346-372` (`TEMP_RENAME_SUFFIX`,
  ...
  130 executable lines protecting a migration both consumers completed (consumer_a `02357b6c0`,
...
- `checks/uid.py:135` `_untracked_sidecars` covers `.gd`-without-`.uid` for TRACKED files only; an
  untracked/staged new `.gd` — the moment of risk — is invisible (consumer_b's written ask:
...
- Port Godot's uid base36 codec (`text_to_id`/`id_to_text`) to Python so a non-canonical TEXT
  spelling of a valid id becomes a `check uid` finding (consumer_a's 155-line
...
- Consumer_b's written ask (`tools/dev/checks/content_schema_lint.py:22-27`): `scene` exposes
...
- Consumer_b: no `make pm` target and no shim despite CLAUDE.md:183 mandating pm-through-CLI — copy
  consumer_a's 19-line `tools/dev/devkit` shim + `make pm`. No `defaults-scan` target (consumer_a
...
- After uid-deepening ships in a release both repos pin: delete `cc-uid-sidecar.sh` (both) and
  consumer_a's `tools/dev/checks/resource_uid_scan.sh` (155 lines).
- Consumer_a devkit targets/claims were audited accurate; keep as the reference wiring.
