...
`devkit.toml`s grepped for retired keys (zero hits in consumer_a and consumer_b). Not verified:
...
`src/agentic_sdlc/repo/installables/pm-operator.md:42-54`. The tooling contract shipped to
...
`src/agentic_sdlc/repo/pm/templates/bug.md:15-18`: "An open bug left under a closed
...
`src/agentic_sdlc/repo/install.py:140` (`_NEXT_STEP['install-agents']`, printed to every
consumer after an install): "The SDLC these agents run is docs/AGENT_WORKFLOW.md in the
agentic-sdlc repo." The file is root-level **`SDLC.md`** (there is no `docs/AGENT_WORKFLOW.md`
...
`src/agentic_sdlc/core/apply.py:340-341` (`move`) has **zero** readers in `src/` and zero
...
`src/agentic_sdlc/repo/checks/pm.py:61-245`. Linear and readable, but four distinct
...
`src/agentic_sdlc/godot/checks/uid.py:272-375` (104 lines) is a clean orchestrator, but
...
grain count (consumer_a: 158+ grains). Proposal: cut the unread keys from the packaged
...
`src/agentic_sdlc/repo/installables/cc-godot-sandbox.sh:136-140`. Today
...
`tools/dev/_common.sh` + the consumer_a make-target roster. The hook-corpus-upstream story
...
- CLAUDE.md:43: "New check = … + branch in **`_run_check`**" — the branches live in
  `_dispatch_check` (`src/agentic_sdlc/cli.py:190`) since the `--fix` split; `_run_check`
