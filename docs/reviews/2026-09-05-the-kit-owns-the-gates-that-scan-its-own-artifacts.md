...
| K1 | BLOCKER | landed 2a23d84 — `SkipReason.UNREADABLE_DIR` (walk.py:95) is recorded by the sweep at :314 and is in `is_unexamined` (:120), riding the second `rglob('*')` the symlink case already paid for, as the finding's own minimum shape asked. Re-measured on two scratch trees with a mode-000 `pm/roadmap/0.1.0/stories/`: zero census → `FAIL — 0 PM document(s) …, 1 dir(s) NOT READABLE by this process and so not descended, and this kept nothing while leaving 1 entr(ies) unexamined`, EXIT=1. Non-zero census (one readable milestone beside it) → PASS, EXIT=0, and the narrowing IS now in the census line — the quieter half of the finding, disclosed the way the symlink case is |
| K2 | MINOR | landed — the Close block is corrected in place. Verified: `git log -- src/agentic_sdlc/repo/checks/grain_shape.py` is 6e9388d + cc0569d ONLY, and `git show cc0569d -- …/grain_shape.py` carries both `THE CENSUS GOES IN BOTH LINES` and the `if found.unexamined():` FAIL. `git show --name-only 14dca39` is milestone.md + walk.py, which is M2. The story now names all three hashes against what each carries. The milestone record's own `| M1 | BLOCKER | landed 14dca39 |` row is where the error propagated from and is still wrong; it is not this record's to amend |
| K3 | MINOR | landed — reported, not refused, which is D2's ruling: D2 accepted this exact cost in writing and required only that the census stay LOUD ON ZERO, and refusing a corpus for being small is this package deciding for a consumer (rule 9). Zero was already loud (`NO CORPUS`, hooks.py:308). How FEW was not: `2 replay their own --self-test corpus` read as coverage of the guards. The verdict line now splits it from a shape probe (`BLOCKS_DECL`, a non-comment `exit 2` line — derived, never a roster). Measured on this repo: `[check:hooks] PASS — armed at tools/hooks; 7 hook(s) …, 2 replay their own --self-test corpus, NONE of the 3 that can BLOCK (exit 2) — the replay covers only hooks that always allow`. Three new cases in tests/test_check_hooks.py, including the other direction (give a blocker a corpus → `1 of 3 that can BLOCK`) so the sentence cannot be a constant |
```
