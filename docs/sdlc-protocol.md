...
| 5 | `telemetry-live` | — *(reads the tree)* | the ledger couriers are wired in `.claude/settings.json` AND their own `--self-test` passes against this tree's vehicle — a probe, not a file read. Never mandatory: a tree that has opted out is quiet, not broken. |
