...
`shellcheck -x` clean on all three installables under bash 3.2.57. Zero `consumer_a`/`consumer_b`
...
header documents "read GDK_GATE_EXIT, never `$?`" but not "never `-e` with pipefail". consumer_a's
...
- `install-runners` writes both files `-rw-r--r--`; `import_cache.sh`'s header spells its usage
  ...
  does not (consumer_a's target runs `bash …`, so it works there).
...
- S4's consumer half (consumer_a/consumer_b install, fork deletion, verdict-line diff) — pending the
