---
id: 0.2.0/bugs/the-deprecation-window-must-close
milestone: "0.2.0"
...
# the-deprecation-window-must-close

## Version note — the numbers moved when the package split

The window shipped in **godot-devkit v0.24.0**, which is where the PM machinery lived at the time. That
code is now this package, inherited at **agentic-sdlc 0.1.0** with the window still open. **This
package's 0.2.0 is the release that trims it.** Where the text below says "0.25.0 removes them", read
"agentic-sdlc 0.2.0"; where it says 0.24.0, that is the historical release that opened the window and
the reference is correct as written.
...
Measured on 2026-09-04, after the change: **consumer_b** 25 grains (`todo` ×24, `review` ×1), **consumer_a**
...
- **consumer_b's 8 `status: active` documents** under `features/*/plans/`, which the tree walk never
