...
`randomize()`); a probe passed them silently. Census-exact parity with consumer_a's `rng_scan.sh`,
...
the review run, on a consumer_a file its own developer deleted mid-run.
...
**NIT-4 — `install-runners` on consumer_a / consumer_b refuses without `--force`** (both own
...
scaffolding — consumer_a, 2026-09-02, three POs and the orchestrator each did it by hand. Expected:
