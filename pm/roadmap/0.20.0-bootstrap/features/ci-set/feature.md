...
devkit-owned files. Release, website, and social workflows (consumer_a's `release.yml`,
...
are deleted; consumer_a's `build-check.yml` and consumer_b's `parse-and-smoke.yml` are either the
