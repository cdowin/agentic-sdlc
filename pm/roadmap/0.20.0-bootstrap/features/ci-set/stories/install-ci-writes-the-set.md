...
## Port
consumer_a + consumer_b `.github/workflows/{auto-tag,semver-gate,uid-guard}.yml` — diff the two copies first; where they differ, the devkit version is the union with a documented variable, never a project name.
## Verification
...
done: 60ad75c, b40af10 — the four workflows on `install-ci`. Unions: consumer_b's
