...
## Steps
`agentic-sdlc install-runners --force` in each; delete `tools/dev/checks/parse.sh|lint.sh|warnings.sh` and `tools/dev/runners/*.sh` that the install replaced; Makefile targets point at the installed paths; `make precommit` (nullbound) / `make check` (trail); `make consumer-smoke`.
## Commit prefix
