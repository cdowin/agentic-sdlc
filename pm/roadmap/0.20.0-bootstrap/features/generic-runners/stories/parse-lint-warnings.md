...
## Port
consumer_a `tools/dev/checks/parse.sh` (boot-clean sweep + `compile_sweep.gd`), `lint.sh` (gdlint), `warnings.sh` — consumer names → `GDK_*` variables with defaults. The `.gd` sweep script travels as an installable too.
## Verification
...
`GDK_PARSE_SWEEP_SCRIPT`. Self-tests: parse 10, lint 8, warnings 8 cases; zero `consumer_a`/`consumer_b`
