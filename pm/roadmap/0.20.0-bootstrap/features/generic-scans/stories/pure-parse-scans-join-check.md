...
## Port
consumer_a `tools/dev/checks/rng_scan.sh` + `rng_allowlist.txt`, `tres_comment_scan.sh`, `unit_disk_scan.sh`, `test_shape_scan.sh` + `test_shape_allowlist.txt` → Python under `repo/checks/`, allowlists as `devkit.toml` sections. Same exit semantics as the existing gates (0 clean / 1 findings / 2 config).
## Verification
...
cases; parity vs the shell scans at consumer_a d3df6e5cb exact on every census.
