...
milestone: "0.20.0"
name: agentic-sdlc init writes every installable in order on an empty Godot 4 project
status: done
owner: developer
depends_on: []
---

# agentic-sdlc init writes every installable in order on an empty Godot 4 project

## Goal
`agentic-sdlc init` composes `pm init` + the `install-*` verbs + the two new files (`devkit.toml` template, two-line `Makefile`) + `.gitignore` entries + a `CLAUDE.md` skeleton, idempotent, `--force`/`--diff` like the installers.
## Verification
...
done: c7e58e3 — `agentic-sdlc init` composes `pm init` + the four `install-*`
