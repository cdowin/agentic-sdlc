...
`Makefile` is `include Makefile.devkit` + its own targets. Extra gates (consumer_a's 19 scans) join
`check` through `[gates] extra = [...]` in `devkit.toml`, read by the include — extension by config,
never by fork.

## Existing-construct audit

The pinned `DEVKIT := uvx --from "git+…@$(DEVKIT_VERSION)" agentic-sdlc` line, the `tools/dev/`
...
Consumer_a's Makefile drops from 59 targets to the include + its own; consumer_b's likewise; `make
