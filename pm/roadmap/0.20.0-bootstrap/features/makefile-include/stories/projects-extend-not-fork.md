...
## Goal
consumer_a's and consumer_b's Makefiles become `include Makefile.devkit` + their own targets; consumer_a's 19 scans join `check` via `[gates] extra`; `make help` shows both sets; `make precommit` green in both; `consumer-smoke` green.
## Commit prefix
