...
- **Known, not fixed here:** the five `[shell] roots` config tests assert exit 2 but see SKIP on a
  host without shellcheck (pre-existing); the consumer smoke against consumer_a is red once
  shellcheck is present because consumer_a's own scripts carry findings (38 scripts) — a consumer_a
