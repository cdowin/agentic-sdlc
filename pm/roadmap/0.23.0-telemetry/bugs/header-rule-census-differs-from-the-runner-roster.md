...
milestone: "0.23.0"
name: check test-shape scans git ls-files tests/integration minus infra basenames (160 on consumer_a) while integration.sh discovers via find minus
status: fixed
caught_in: "0.22.1"
...
MINOR — check test-shape scans git ls-files tests/integration minus infra basenames (160 on consumer_a) while integration.sh discovers via find minus */support/* and _capture$ (137) — the opt-in header rule demands Boots-because/covers on 22 *_capture.gd tools and a support stub that --diff can never slice to. One roster, owned by one place, read by both. From the v0.22.0 release review.
