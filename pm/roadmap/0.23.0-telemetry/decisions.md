...
reports; the `consumer_a-baseline` story is dropped and the first hook-captured milestone is the
...
named here. The worktree path still says `agentic-sdlc-0.22.0`; a path is not an id.

## D10 — 2026-09-03 — consumers-adopt leaves this milestone: a consumer adopts at its own pin bump, in its own PM tree

**Decision:** the `consumers-adopt` feature and its `consumer_b-adopts` story are removed from 0.23.0.
...
That work is a story in the consumer's PM tree (consumer_b's, at its 0.28.4 close), not a feature here.
**Because:** `pm milestone done` refuses while any feature is not `done` (D3), and a feature whose
ship criterion is "consumer_b's next milestone reports from live capture" cannot be done before the release
...
is not); keeping the milestone open until consumer_b adopts.
