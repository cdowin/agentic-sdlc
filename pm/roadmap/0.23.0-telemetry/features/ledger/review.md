...
`pm new`, never against `~/workspace/consumer_b` or `~/workspace/consumer_a`.
...
wrote.** `src/agentic_sdlc/repo/pm/ledger.py:425` (`row_names`).
...
- Behaviour against the live consumer checkouts (`~/workspace/consumer_b`,
  `~/workspace/consumer_a`) — deliberately untouched.
