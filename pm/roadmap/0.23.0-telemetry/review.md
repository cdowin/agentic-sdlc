...
+14,571 / −125), in the `agentic-sdlc-telemetry-int` worktree. Method: every new and changed source
...
running it — against a scratch **clone** of consumer_a (145 MB `.git`, `--local`, nothing written to
`~/workspace/consumer_a` or `~/workspace/consumer_b`), against real Claude Code transcripts under
...
**On the consumer_a clone (pin v0.22.0, `header = true`, one milestone building):** `check test-shape`
...
(1 refused: "no assistant record", exit 2); `pm ledger report` over consumer_a's 26 review records
...
`src/agentic_sdlc/godot/checks/test_shape.py:211-212`. The docstring at :120-130 promises "Raises
...
append `integration-list:\n\t@true` to consumer_a's Makefile, empty `header_ledger` →
...
`src/agentic_sdlc/repo/pm/verdict.py:103,135` — `DISPOSITION_KINDS = (landed, rejected, deferred)`;
...
`src/agentic_sdlc/repo/pm/skills.py:102-159` — `install_merge_attribute` runs from `cmd_init` only.
Neither consumer has a `.gitattributes` (checked: consumer_a, consumer_b). CHANGELOG bullet 1 says "`pm
...
names (consumer_a: 13), (3) `pm init` once for the attribute, (4) optionally `install-hooks` + the
...
`src/agentic_sdlc/repo/install.py:567-574`. On the consumer_a clone: `--diff` shows the two couriers as
...
`prepare-commit-msg`); consumer_b's tree has the same shape. The documented way through is `--force` then
...
- Consumer_b beyond facts: pin v0.20.0, `[test_shape] header` off (no roster impact), hooks customized,
...
consumer_a's `make check` is red after the bump until `install-runners --force` (else exit 2) and 13
`header_ledger` lines are dropped (else exit 1), and `pm init` once (else merge conflicts on the
ledger); consumer_b needs only the `pm init`. The Makefile itself is untouched, which is where the
major line is drawn.
