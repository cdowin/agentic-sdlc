...
- **consumer_a: 194 scenes, 0 failures.** The row widens there today.
- **consumer_b: 116 scenes, 13 failures**, none of them the marker.
...
plainly gives it. Live on consumer_b: `scenes/moments/force_resolution.tscn` (`PaperBg` 0,
`Payload` 2) and `scenes/moments/game_over.tscn` (`Payload` 2). consumer_a has 10 inherited
...
- **The opposite direction is in the same repo, the same week.** 4 structurally identical
  ...
  these 4 too.
- **The editor-written half of the corpus says no.** All 116 consumer_b scenes carry a `;` comment,
  so none has been through `ResourceSaver` since it was written; 0 of consumer_a's 194 do. In
  consumer_a — every file editor-written — **74 created nodes sit inside an instanced sub-tree
...
two trees (319 consumer_b, 186 consumer_a) — and still gets 3 of the 12 wrong.
...
|---|---|---|---|---|
| consumer_b, before | 17 | 1 | 0 | 3 |
| consumer_b, after | 17 | **4** | 0 | **0** |
| consumer_a, before/after | 39 | 39 | 0 | 0 |

Round-trip census: consumer_b **13 -> 11** failing of 116, consumer_a **0 of 194** throughout.
...
comment census above), not a counter defect. **Nothing was changed in consumer_b**: this package
does not write into a consumer, and the repair is consumer_b's to make.
...
## The widening is blocked on consumer_b, and it is 11 files, not 1

`make smoke`'s `canonicalize round trip` row stays pinned to one scene per consumer. It can
widen on consumer_a (194/194 today) and cannot on consumer_b, where 11 of 116 still fail: 10 for the
...
- **consumer_b repairs its 11 files** — drop the 9 created-node indexes that are already the slot
