...
Found by consumer_b adopting v0.23.0 (2026-09-04): `install-ci`'s `verify.yml` is checkout → `setup-uv`
...
PR, and the red says nothing about the consumer's code. consumer_a's copy works only because it was
...
`src/agentic_sdlc/repo/installables/ci-verify.yml` is this package's OWN CI shape (a Python package
...
both trees: a `setup-godot` step (consumer_a's shape) whose version is a project-config value at the
...
`install-ci --diff` → `--force` and re-applying nothing (consumer_a's hand edit becomes the shipped
