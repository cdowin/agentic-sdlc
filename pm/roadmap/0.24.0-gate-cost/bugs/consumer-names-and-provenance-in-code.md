...
> *"delete/remove ANYTHING that knows anything about consumer_a or consumer_b. This is a completely project
...
Chris, 2026-09-02: *"comments that reference things like 'consumer_b's copy was the 115-line ancestor'.
We need to have code be project-agnostic. Comments and code should just be what is, and should
describe the utility."* Measured on `main` at v0.20.0 (word-bounded `consumer_a`/`consumer_b`, outside
`pm/` and `CHANGELOG.md`, which are history):

- `src/agentic_sdlc/repo/gates_extra.py` module docstring — "consumer_a has …"
- `src/agentic_sdlc/repo/install.py` — the `install-hooks` next-step text twice: "(consumer_a: a
  `hooks-self-test` target in …)"
- `README.md` — the `install-hooks` row cites a consumer's target by name
- 8 test files, 10 docstring/comment hits (e.g. `test_install.py` — "consumer_b chmods `cc-*.sh` by
  glob, consumer_a named two files") that explain a test by which consumer did what
