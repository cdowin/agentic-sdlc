...
**consumer_b repaired the three mismatches** at `55e52a79` ("three stale body indices against
...
|---|---|---|---|---|---|---|
| consumer_b | 116 | **10** | 17 | 5 | 0 | 12 |
| consumer_a | 194 | **0** | 39 | 39 | 0 | 0 |
...
Ten for, ten against, same repo, same position, nothing structural between them. And consumer_b cannot
...
**consumer_a is the engine-written half and it says no, loudly, where it can speak: 194 scenes, 1008
...
| rule | consumer_b failing | consumer_b invented | consumer_a failing | consumer_a invented |
|---|---|---|---|---|
| ship today | 10 | **0** | 0 | **0** |
...
The rule **as this bug states it** takes consumer_a from 0 failing scenes to 26 and invents 125
attributes; with the plausible fallback it invents 500, which is the 505 the sibling bug measured.
The narrowest form buys consumer_b 10 failures -> 3 and invents exactly the four this bug named as its
...
- `_restore_indexes` keeps the skip and its stated reason is now the census above; the previous
  comment cited the three mismatches consumer_b has since repaired.
...
Re-save **`consumer_b/scenes/moments/force_resolution.tscn`** in the Godot editor — better than one of
...
|---|---|
| `PaperBg`/`Payload` keep `index=`; `MontageView`/`ResultsView`/`Timer` gain `4`/`5`/`6`; the nodes under `MontageVBox` gain indexes too | the engine indexes EVERY non-root node of an inherited scene. This bug is real and much larger than filed: consumer_b's inherited scenes are ~200 attributes short, and the tool should write all of them. |
| the three gain `4`/`5`/`6`, deeper created nodes gain nothing | the narrowest rule is right and the four are consumer_b data bugs. Restore, and consumer_b repairs those two files. |
...
`make smoke`'s `canonicalize round trip` row stays one scene per consumer. It can widen on consumer_a
(194/194) and cannot on consumer_b, where 10 of 116 fail — all ten of them a created node's authored
`index=`, none of them a defect this package can fix. If consumer_b dropped those ten attributes the row
...
- **The tool is correct today by the only evidence available.** consumer_a is the engine-written half
...
- **The refused change was the risky one.** Implementing the rule as filed took consumer_a from 0
...
`consumer_b/scenes/moments/force_resolution.tscn`. Its four outcomes are already mapped to four different
