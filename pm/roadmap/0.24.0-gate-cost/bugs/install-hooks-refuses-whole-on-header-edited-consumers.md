...
v0.23.0 release review m1. On consumer_a and consumer_b `install-hooks --diff` shows the two ledger couriers
...
`prepare-commit-msg`). The way through was `--force` then `git checkout --` the four (consumer_a
`c402619eb`), or hand-copying two files out of a uv cache.

## Root cause

`src/agentic_sdlc/repo/install.py` (~:567-574): one collision anywhere in the roster refuses the whole
