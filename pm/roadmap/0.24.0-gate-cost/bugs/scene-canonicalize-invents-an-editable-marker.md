...
— 13 in consumer_b, 8 in consumer_a. In each, canonicalizing a degraded copy produces a file carrying an
`[editable path="…"]` section the original did not have. Example: consumer_a
...
26 invented markers (25 consumer_a, 1 consumer_b) before, **0** after, with **0** `[editable]` lines
removed anywhere.
...
- **consumer_a 194 scenes, 0 failures.** The row can widen there today.
- **consumer_b 116 scenes, 13 failures** — and none of them is the marker. Two shapes:
  - **12: `index=` is never restored on a node that carries `type=`.** All 12 are INHERITED scenes
    ...
    never put back. consumer_a has no inherited scenes, which is why this is consumer_b-only.
...
So the round-trip row widens to every tracked scene on consumer_a now, and on consumer_b only after the
