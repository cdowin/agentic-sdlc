...
Five unit modules call `tests/support.available_consumers()` and walk `~/workspace/consumer_b` and
`~/workspace/consumer_a` — `test_defaults` (one 12 s test), `test_check_props` (3.4 s), and
