...
roster through `make integration-list`, consumer_a had opted into the header rule, its runners were
v0.22.0, and the smoke was red on a consumer state the release's own adoption note says to leave
(consumer_a `0.90.3.2` M48 — the fix was to advance the fixture by hand, ahead of the pin). When
...
`consumer_smoke.py` already imports `agentic_sdlc.repo.install` for the fresh project; the same
