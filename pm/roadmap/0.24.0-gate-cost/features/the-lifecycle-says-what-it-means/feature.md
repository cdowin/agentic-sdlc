...
**Not ours:** consumer_b carries 8 grains at `active`, a status in no devkit default set — pre-existing
drift in that repo, surfaced by this census and left for consumer_b's own session.
...
2. **The consumer migration is a follow-up, not part of this.** consumer_a holds 38 `todo` and 8
