...
- Branch **`milestone/0.24.0-gate-cost`**, trunk at `~/workspace/agentic-sdlc`.
...
- `uv run python -m agentic_sdlc.cli pm status 0.24.0` is the scoreboard.
...
- Bump `DEVKIT_VERSION`. **consumer_a must not bump until the tag exists** (M48 caught the reverse).
- `install-ci --diff` then decide **per file**. **Never blanket `--force`** — it rewrites all four
  workflows with no per-file option, and it would replace consumer_b's deliberate two-job sharded
  `verify.yml` (177 lines of difference) and drop a path filter from consumer_a's `auto-tag.yml`.
- `install-hooks`, `install-runners`, one `pm init` re-run.
- **Migrate `status:` lines.** Measured 2026-09-04: consumer_a 37 story `todo` → `ready` and 9 story
  `review` → `reviewing`; consumer_b 24 and 1. **No feature in any tree holds `review`.** consumer_b's 8
  `active` grains live in `features/*/plans/*.md`, a slot the PM walker never visits — `check pm` in
  consumer_b exits 0 today and the new set does not change that.
