...
gate-semantics claim probed by RUNNING it — against a `--local` clone of consumer_b (38 MB `.git`),
...
copies of `src/` for the canonicalize guard. Nothing was written into `~/workspace/consumer_b` or
`~/workspace/consumer_a` (both verified `git worktree list`-identical and `git status`-clean after
...
Makefile, not consumer_a (`make: *** No rule to make target 'hooks'`); only this repo's own root
...
**On the consumer_b clone (pin as installed, `--local` clone of `~/workspace/consumer_b`):** the full
...
index-identical)` — the same five numbers its table states for consumer_b.

## MAJOR

### M1 — `check hooks` names `make hooks` as its repair, and no consumer has that target

`src/agentic_sdlc/repo/checks/hooks.py:63` (`ARM_COMMAND = 'make hooks'`), printed in four findings
...
in **this repo's own root `Makefile`** (added by 5e1320d) and nowhere this package ships:
    ...
    $ grep -nE '^[a-z][a-z0-9-]*:' src/agentic_sdlc/repo/installables/Makefile.devkit | grep hooks
    ...
    $ make -C ~/workspace/consumer_a -n hooks
    make: *** No rule to make target `hooks'.  Stop.

    $ (cd <consumer_b clone> && agentic-sdlc check hooks)
...
consumer_b passes only because consumer_b hand-rolled its own `hooks:` target years ago; consumer_a has none,
and neither does the `Makefile` that `agentic-sdlc init` writes (`project-Makefile` is
...
opt-in — `agentic-sdlc check hooks` is in the CLI help this release added (cli.py:87) and
...
1. **The census reproduces.** My independent run of the smoke row on the consumer_b clone returned consumer_b
...
3. **The direction is the safe one.** The refused rule takes consumer_a from 0 failing scenes to 26
...
- The smoke against **consumer_a**: read only. An agent is working in that checkout and its tree
  moves; I ran the full 18-row smoke against a consumer_b clone instead, and only read consumer_a's
  `Makefile`, `project.godot` and `git worktree list`. So the consumer_a halves of the CHANGELOG's
...
- Whether Godot **4.6.2** — `GODOT_PATCH: "2"` against consumer_a's and consumer_b's declared `4.6` — is
...
`check all` on the consumer_b clone is exit 0 through the release's own runners). `Makefile.devkit` is
...
hooks-self-test matrix smoke`. Measured with `PYTHONPATH=<devkit>/src python3 -m agentic_sdlc.cli`,
byte-identical to `consumer_smoke.devkit()`:

```
consumer_b      pm status 0 · pm validate 0 · check pm 1 (25 findings) · check all 0
consumer_a  pm status 0 · pm validate 0 · check pm 1 (47 findings)
```

consumer_a's `[checks] all` names `pm`, so its worktree `check all` reds too. consumer_b's default roster
excludes `pm`, but consumer_b's `[gates] extra` runs `check pm` in `make check` — so **both consumers'
pre-push gates red on pin bump**.

The wedge, proven on a scratch copy of consumer_a's tree:
...
because `consumer_smoke.CONSUMERS` is `~/workspace/{consumer_b,consumer_a}` and absent consumers SKIP — so
...
`src/agentic_sdlc/repo/pm/report.py:1226-1244`. `reopenable` now tests `model.REVIEWING in
...
unattributable. The `-`s are honest; the `0` is not. consumer_a's live `0.90.3.2/ledger.jsonl` holds
...
- **m1 — the shipped migration instruction is already false, and covers stories only.** `CHANGELOG.md`
  ...
  feature in any tree holds `review`."* Measured: consumer_a holds
...
consumer_b's 8 `active` grains live in `features/*/plans/*.md`, a slot `slot_walk` never visits — the new
...
consumer_a (peer agents were live in that tree — composed from `check pm` exit 1 plus `pm` being named
