...
| | |
|---|---|
| **Branch** | `milestone/0.10.0-the-ledger-is-a-stamp` (the stamp in `branch:`). **Not created yet**: cut it from `main` AFTER v0.9.0 is merged and tagged |
| **Version** | 0.10.0, bumped at CLOSE (D8 is off here). `__version__` is 0.9.0 until the release commit |
...
0.9.0 (`ms-the-tool-agrees-with-itself`, a lean bug-and-debt release minted 2026-09-11) must ship first.
It displaced this milestone from 0.9.0 and took #39's double-count half. Then:

```bash
git checkout main && git pull --ff-only && git checkout -b milestone/0.10.0-the-ledger-is-a-stamp
...
confinement and `dispatch` learning the mode, so the rest of 0.10.0 runs under its own guards.
