...
#
# The dependency roster below is the stock agentic-sdlc consumer dev loop
...
# --- uv (the agentic-sdlc gates) — CRITICAL ----------------------------------
# The devkit gates (check uid/tres/doc/… + the introspect targets) run the
# pinned agentic-sdlc package via uvx, so a missing uv kills those rungs.
if command -v uv >/dev/null 2>&1; then
	pass "uv $(uv --version 2>/dev/null | awk '{print $2}')"
else
	fail "uv not on PATH" \
	     "brew install uv — the devkit gates invoke the pinned agentic-sdlc via uvx."
fi
...
if [ -d "$HOOKS_PATH" ]; then
	...
	[ "$hooks_seen" -eq 1 ] || warn "no tracked hooks under $HOOKS_PATH/" \
	     "agentic-sdlc install-hooks ships the guard corpus; bash tools/setup-hooks.sh arms it."
else
