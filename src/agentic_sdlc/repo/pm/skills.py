...
# them: the name is FREE TEXT, so it is single-quoted at both parses (D2).
FIRST_MILESTONE = (
    vehicle.command('pm', 'new', vocabulary.GRAIN_MILESTONE, 'first-light',
                    'First Milestone', '--version', '0.1'),
    vehicle.command('pm', 'validate'),
)
...
def stand_up_tree(cfg: vocabulary.PmConfig) -> list[str]:
...
def attribute_pattern(cfg: vocabulary.PmConfig) -> str:
...
def install_merge_attribute(cfg: vocabulary.PmConfig) -> str:
...
def install_flow(cfg: vocabulary.PmConfig) -> str:
    ...
    if defect:
        raise Refused(f'{CONFIG_FILE} {defect} — the flow was not written; '
                      f'run `{vehicle.command("pm", "vocabulary")}` for the '
                      f'section and '
    ...
    eol = '\r\n' if '\r\n' in existing else '\n'
    block = (FLOW_HEADER + vocabulary.render_seed()).replace('\n', eol)
    if existing:
        head = '' if existing.endswith(('\n', '\r')) else eol
...
def cmd_init(cfg: vocabulary.PmConfig, args: list[str]) -> int:
...
def cmd_config(cfg: vocabulary.PmConfig, args: list[str]) -> int:
