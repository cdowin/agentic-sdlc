...
def test_every_installable_on_disk_is_reachable_through_a_verb():
    ...
    # The sixth installer's payloads, the same question of their directory.
    from agentic_sdlc.repo.pm import skills
    found = walk.children(REPO_ROOT / 'src/agentic_sdlc/repo/pm/guidance',
                          Kind.FILE)
    on_disk = {p.name for p in found.kept}
    named = {name for name, _ in skills.GUIDANCE_PLAN}
    assert on_disk == named, (
        f'guidance unreachable: {sorted(on_disk - named)}; '
        f'missing: {sorted(named - on_disk)}')


def test_this_repo_carries_the_guidance_it_installs_current():
    """Every `pm install-skills` destination is here, current with its source.

    Nothing graded these: the three installed copies sat at a v0.7.0 stamp
    under a v0.8.0 package, and a skill dropped from this repo would have
    gone unnoticed. The STAMP is which release installed the file, not its
    content, so it is read out of both sides; everything else is the
    installer's own verdict — byte-current, or different only inside the
    `## Project config` block this repo may edit."""
    from agentic_sdlc.repo.pm import skills
    stamp = re.compile(r'(GENERATED by agentic-sdlc )v\S+')
    for name, rel in skills.GUIDANCE_PLAN:
        target = REPO_ROOT / rel
        assert target.is_file(), (
            f'{rel} is not installed in this repo — run '
            f'`pm install-skills --force`')
        text = stamp.sub(r'\1', target.read_text(encoding='utf-8'))
        body = stamp.sub(r'\1', skills.guidance_body(name))
        assert text == body or install.header_only_difference(text, body), (
            f'{rel} differs from pm/guidance/{name} — edit the source and '
